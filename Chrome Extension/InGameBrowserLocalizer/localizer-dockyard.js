if (!window.__localizer_dockyard_loaded__ && window.location.host === "dockyard.korabli.su") {
    ...
    detect_elements.add([
        ".ViewDialogs_multipleBuyDialog_39eJ- .RulesDialog_paragraph_3EDf3 .RulesDialog_text_3Ruqd span",
        {
            ...
            translation: [
                ...
                [
                    /Получить финальную награду можно, только завершив не менее 4 любых этапов за дублоны. Если после получения финальной награды вы продолжите выполнять боевые задачи Верфи, то вместо каждого дополнительного этапа вы получите/g,
                    "您需要使用金币完成至少4个阶段才有可能获得最终奖励。如果在获得最终奖励后继续完成造船厂的作战任务，那么每完成一个额外的造船阶段，您将获得以下奖励"
                ],
                [
    ...
    detect_elements.add([
        "#wows-react-tooltip-body .RewardShipTooltip_bgWrapper_GGlA6 .RewardShipTooltip_premiumRow_24dlf span span",
        {
            ...
            isReplaceHTML: true,
            translation: [
                ['Получить корабль можно, только завершив не&nbsp;менее', "您至少要完成以下任务才能获得一艘战舰"],
                ['любых этапов за&nbsp;дублоны.', "通过金币推进任意造船阶段。"],
            ]
        }
    ]);
    detect_elements.add([
    ...
    translation.set(
        ...
        "炮塔装甲安装。尾部鱼雷发射管装配，潜望镜、上层舰桥设备安装，栏杆围栏装配。潜艇服役前整备。"
    );
